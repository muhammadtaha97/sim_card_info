/// Country names and flag emoji from the ISO 3166-1 alpha-2 codes that
/// TelephonyManager and SubscriptionManager report (simCountryIso,
/// networkCountryIso).
library;

/// The flag emoji for an ISO alpha-2 code, computed from the regional
/// indicator symbols rather than looked up, so it cannot go stale.
...
/// Display name for an ISO alpha-2 code; falls back to the upper-cased code
/// itself so an unmapped country still shows something true.
String countryName(String? iso) {
  if (iso == null || iso.isEmpty) return 'Unknown';
  return _names[iso.toUpperCase()] ?? iso.toUpperCase();
}
...
const Map<String, String> _names = {
  ...
  'TO': 'Tonga', 'TR': 'Türkiye', 'TT': 'Trinidad and Tobago', 'TV': 'Tuvalu',
